  ...
  execute(index) {
    ...
    if(this.sys.autoResetEffect){
      if(this.sys.setterNodeInitValueMap && !this.sys.setterNodeGuidMap.has(trans.guid.toString())){
        ...
        callBackFuncMap.set((_transformComp, _anchoredPosition) => _transformComp.anchoredPosition = _anchoredPosition, [trans.anchoredPosition]);
        callBackFuncMap.set((_transformComp, _localRotation2D) => _transformComp.localRotation2D = _localRotation2D, [trans.localRotation2D]);
        callBackFuncMap.set((_transformComp, _localScale2D) => _transformComp.localScale2D = _localScale2D, [trans.localScale2D]);
